...
<head>
    ...
    <title>{{ $company['name'] ?? 'MegaShop' }} - Contact</title>
...
<body>
    <header>
        <div class="header-content">
            <div class="logo">{{ $company['name'] ?? 'MegaShop' }}</div>
        </div>
    </header>
    ...
    <main>
            ...
            <div>
                <div style="background: white; padding: 30px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1); margin-bottom: 25px;">
                    ...
                    <div style="margin-bottom: 25px;">
                        ...
                        <p style="color: #555;">
                            <strong>Général :</strong> {{ $contact['phone_general'] ?? $company['phone'] ?? '' }}<br>
                            <strong>Support :</strong> {{ $contact['phone_support'] ?? '' }}<br>
                            <strong>Ventes :</strong> {{ $contact['phone_sales'] ?? '' }}
                        </p>
                    </div>
                    ...
                    <div style="margin-bottom: 25px;">
                        ...
                        <p style="color: #555;">
                            <strong>Général :</strong> {{ $company['email'] ?? '' }}<br>
                            <strong>Support :</strong> {{ $company['support_email'] ?? '' }}<br>
                            <strong>Ventes :</strong> {{ $company['sales_email'] ?? '' }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p style="color: #555;">
                            Lundi - Vendredi : {{ $hours['monday_friday'] ?? '' }}<br>
                            Samedi : {{ $hours['saturday'] ?? '' }}<br>
                ...
                <div style="background: white; padding: 30px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
                    ...
                    <div style="display: flex; gap: 15px;">
                        <a href="{{ $social['facebook'] ?? '#' }}" style="color: #3498db; text-decoration: none; font-weight: bold;">📘 Facebook</a>
                        <a href="{{ $social['instagram'] ?? '#' }}" style="color: #3498db; text-decoration: none; font-weight: bold;">📷 Instagram</a>
                        <a href="{{ $social['twitter'] ?? '#' }}" style="color: #3498db; text-decoration: none; font-weight: bold;">🐦 Twitter</a>
                        <a href="{{ $social['youtube'] ?? '#' }}" style="color: #3498db; text-decoration: none; font-weight: bold;">▶️ YouTube</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <section style="background: white; padding: 30px; border-radius: 8px; margin-bottom: 40px;">
            ...
            <div style="display: grid; gap: 20px;">
                @foreach(($faq ?? []) as $item)
                    <div>
                        <h4 style="color: #2c3e50; margin-bottom: 8px;">{{ $item['question'] ?? $item['title'] ?? '' }}</h4>
                        <p style="color: #555; margin-left: 15px;">{{ $item['answer'] ?? $item['content'] ?? '' }}</p>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        <section style="background: white; padding: 30px; border-radius: 8px;">
            ...
            <p style="color: #555; margin-bottom: 20px;">Notre siège social est situé au cœur de {{ $company['city'] ?? 'la ville' }}. Vous pouvez nous rendre visite sur rendez-vous.</p>
    ...
    <footer>
        <p>&copy; 2026 {{ $company['name'] ?? 'MegaShop' }} - Tous droits réservés</p>
        <div class="footer-links">
            <a href="/cgv">CGV</a>
            <a href="/contact">Contact</a>
            <a href="#">Mentions Légales</a>
        </div>
        <p><strong>Email :</strong> {{ $company['email'] ?? '' }} | <strong>Téléphone :</strong> {{ $company['phone'] ?? '' }}</p>
    </footer>
</body>
</html>
